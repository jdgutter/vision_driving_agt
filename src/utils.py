...
RESCUE_TIMEOUT = 30
TRACK_OFFSET = 15
DATASET_PATH = 'C:/Users/Jgutt/Documents/deep_learning_data/drive_data'
...
def get_dense_transform(**kwargs):
    transform = []

    if kwargs.get("resize") is not None:
      transform.append(dense_transforms.Resize(kwargs["resize"]))
      if DEBUG_EN:
          print("get_dense_transform: resize enabled %s" % (kwargs["resize"],) )

    if kwargs.get("random_crop") is not None:
        transform.append(transforms.RandomCrop(kwargs["random_crop"]))
        if DEBUG_EN:
          print("get_dense_transform: random_crop enabled %s" % (kwargs["random_crop"],))

    if kwargs.get("random_horizontal_flip"):
        transform.append(dense_transforms.RandomHorizontalFlip())
        if DEBUG_EN:
          print("get_dense_transform: random_horizontal_flip enabled")

    if kwargs.get("color_jitter"):
        transform.append(dense_transforms.ColorJitter(brightness=0.5, contrast=0.1, saturation=0.5, hue=0.1))
        if DEBUG_EN:
          print("get_dense_transform: color_jitter enabled")

    transform.append(dense_transforms.ToTensor())

    if DEBUG_EN:
          print("get_dense_transform: finishing up")

    return dense_transforms.Compose(transform)

def load_data(dataset_path=DATASET_PATH, num_workers=0, batch_size=128, **kwargs):

    transform = get_dense_transform(**kwargs)
